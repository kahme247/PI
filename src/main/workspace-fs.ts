...
export async function workspaceFsListDir(req: { workspaceRoot: string; path?: string; maxEntries?: number }) {
  ...
  try {
    ...
    return { ok: false as const, error: 'not_found' as const, entries: [] as const }
  }
...
export async function workspaceFsRename(req: { workspaceRoot: string; relativePath: string; newName: string }) {
  ...
  if (newExists) return { ok: false as const, error: 'target_exists' as const }
  try {
